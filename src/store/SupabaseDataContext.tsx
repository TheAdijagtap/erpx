...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        supabase.from("proforma_invoices").select("id,invoice_number,customer_id,customer_name,customer_address,customer_gst,date,status,subtotal,tax_amount,total,notes,payment_terms,created_at,proforma_invoice_items(id,item_name,hsn_code,description,quantity,rate,amount,unit),proforma_invoice_additional_charges(id,name,amount)").order("created_at", { ascending: false }).limit(200),
        supabase.from("proforma_products").select("id,name,description,unit,price,hsn_code,created_at").order("created_at", { ascending: false }).limit(200),
      ...
      const mappedPIs: ProformaInvoice[] = (pis || []).map(pi => {
        ...
        return {
          ...
          status: (piAny.status?.toUpperCase() || "SENT") as ProformaInvoice["status"],
  ...
  const addProformaInvoice = async (pi: Omit<ProformaInvoice, "id" | "subtotal" | "sgst" | "cgst" | "total"> & { applyGST?: boolean; gstRate?: number }) => {
    ...
    const newPI: ProformaInvoice = {
      ...
      ...totals,
      status: (pi.status || "SENT") as ProformaInvoice["status"],
      date: pi.date,
      notes: pi.notes,
